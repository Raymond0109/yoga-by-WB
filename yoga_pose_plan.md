...
5. ✅ **匹配/评分算法校准 + 时序平滑 + 几何校验 + 反馈排序（v0.5.4）**：
   - 连续评分（`core/pose_compare.py`）：每条规则给连续分（非二值 ok/off），`compare` 返回按 |偏差| 从大到小排序的 `items` + `total_dev` + `low_score_tip`（分数<60 时给出「先纠正最差项」提示）。
   - 时序平滑（`core/landmark_smoother.py`）：One Euro Filter + 关键帧锁定，抑制实时抖动。
   - 几何自检（`core/geometry_check.py`）：骨长/对称性合理性校验，过滤明显异常的 landmark。
   - **视频数据校准（Yoga_base_ref）**：从 502 关键帧 + 命名参考图自建 183 条参考姿态（24 体式，`data/ref/`），`tools/build_ref_set.py` → `calibrate_from_images.py` → `calibration_diff.py` → `calibration_merge.py` 全流水线。**解剖安全合并策略**：目标角仅在与专家顺位收敛（Δ≤8°）时微调（拒绝把直腿/直角掰弯的视频偏差），容差只放宽（never tighten）吸收真实变化，`vertical_order` 的 `min_sep`（阈值非容差）保持不动。最终 6 处目标微调 + 31 处容差放宽，全部 24 单测通过。
   - 交叉校验 `tools/calibrate_crosscheck.py` + 报告 `docs/calibration_diff.md` / `calibration_crosscheck.md`。
